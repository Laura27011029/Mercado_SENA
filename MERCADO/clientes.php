    ...
    <table border="1">
        ...
        <?php while ($row = $result->fetch_assoc()) : ?>
            <tr>
                ...
                <td>
                    <a href="clientes.php?editar=<?= $row['id_cliente'] ?>&nombre=<?= urldecode($row['nombre_cliente']) ?>&email=<?= urlencode($row['email_cliente']) ?>&telefono=<?= urlencode($row['telefono_cliente']) ?>">Editar</a>
                    <a href="clientes.php?eliminar=<?= $row['id_cliente'] ?>" onclick="return confirm('¿Seguro que desea eliminar este cliente?')">Eliminar</a>
                </td>
            </tr>
            <?php endwhile; ?>
        </table>
